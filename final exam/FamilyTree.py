...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        while temp2 != self.root:
            ...
            blist.append(temp2)
        if a_node in blist or b_node in alist:
            return (-1, abs(len(alist)-len(blist)))
        
        len_min = min(len(alist), len(blist))
        
        for i in range(len_min):
            if alist[-1-i] != blist[-1-i]:
                return (len_min-i, abs(len(alist)-len(blist)))
            
        return (0, abs(len(alist)-len(blist)))
